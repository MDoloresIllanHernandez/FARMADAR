  ...
  <header class="absolute inset-x-0 top-0 z-50 bg-gray-100">
    ...
    <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
      ...
      <div class="flex lg:hidden">
        <button type="button"
          class="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-primary-oscuro"
      ...
      <div class="hidden lg:flex lg:gap-x-12">
        <span v-for="item in navigation">
          <a v-if="item.name != 'Usuarios' || (item.name == 'Usuarios' && (roleUsuario != 'usu' && roleUsuario != 'Usuario'))"
            :key="item.name" :href="item.href" class="boton-navbar">{{ item.name }}</a>
        </span>
      </div>
      <div class="hidden lg:flex lg:flex-1 lg:justify-end">
        ...
        <!-- Cerrar Sesión -->
        <a :href="cerrarSesion.href">
          <img src="../assets/shutdown.png" alt="Icono Cerrar Sesión" class="h-6 w-6 rounded-full hover:bg-white" title="Cerrar Sesión" /> 
        </a>
      </div>
    </nav>
    ...
    <Dialog class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
      ...
      <DialogPanel
        class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-primary-oscuro/10">
        ...
        <div class="mt-6 flow-root">
          <div class="-my-6 divide-y divide-gray-500/10">
            <div class="space-y-2 py-6">
              <span v-for="item in navigation">
                <a v-if="item.name != 'Usuarios' || (item.name == 'Usuarios' && (roleUsuario != 'usu' && roleUsuario != 'Usuario'))"
                  :key="item.name" :href="item.href" class="boton-navbar-movil">{{ item.name }}</a>
              </span>
